...
function getGR3DTestsData(target: Target) {
  ...
  return {
    binary: `gr3d-tests.${binExts[target]}`,
  };
}

function getExportTemplatesDir(osName: OSName, version: string) {
  const trail =
    osName === "windows"
      ? "/AppData/Roaming/Godot/export_templates"
      : osName === "macos"
      ? "/Applications/Godot.app/Contents/Resources/templates"
