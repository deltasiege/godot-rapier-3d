...
static var Actions = preload("./actions.gd")
...
static func on_received_input_tick(sync: GR3DSync) -> Callable:
	return func(peer_id: int, serialized_msg: PackedByteArray):
		...
		print("next_local_input_tick_requested ", peer.next_local_input_tick_requested)

		if first_remote_tick >= sync.input_tick + sync.max_buffer_size:
		...
		if last_remote_tick > peer.last_remote_input_tick_received:
			...
			for remote_tick in all_remote_ticks:
				...
				if remote_tick < sync.action_buffer_start_tick:
					continue

				var remote_input = sync.message_serializer.unserialize_input(all_remote_input[remote_tick])

				var input_frame := Actions.get_or_create_ab_frame(sync, remote_tick)
				if input_frame == null:
				...
				var tick_delta = sync.current_tick - remote_tick
				if tick_delta >= 0 and sync.rollback_ticks <= tick_delta:
					...
					if local_input['$'] != remote_input['$']:
						sync.rollback_ticks = tick_delta + 1
						sync.prediction_missed.emit(remote_tick, peer_id, local_input, remote_input)
						sync.rollback_flagged.emit(remote_tick)
				else:
			...
			var index = (peer.last_remote_input_tick_received - sync.input_buffer_start_tick) + 1
			while index < sync.action_buffer.size() and not sync.action_buffer[index].is_player_input_predicted(peer_id):
